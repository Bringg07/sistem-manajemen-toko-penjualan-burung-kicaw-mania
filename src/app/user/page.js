...
import { consumeAvailableAlerts } from "@/lib/wishlist";
import {
  Plus,
  Edit,
  Trash2,
...
export default function KatalogPage() {
  ...
  // 4. Tutup modal/drawer dengan tombol Escape
  useEffect(() => {
    function onKey(e) {
  ...
  // Debounce input pencarian (400ms)
  useEffect(() => {
  ...
  // 5. Fetch katalog dari API (pagination + filter di server)
  const fetchBirds = useCallback(async () => {
    try {
      ...
      if (selectedCategory !== null) params.set("kategori", String(selectedCategory));

      const res = await fetch(`/api/birds?${params.toString()}`);
      if (!res.ok) throw new Error("Gagal memuat katalog.");
      const json = await res.json();
      setBirds(json.data || []);
      setBirdCategories(json.categories_by_bird || {});
      setTotalPages(json.total_pages || 0);

      // Notifikasi: burung favorit yang dulu stoknya habis kini tersedia
      const available = consumeAvailableAlerts(json.data || []);
      available.forEach((b) => showToast(`${b.name} kini tersedia!`, "success"));
    } catch (err) {
  ...
  // 6. Daftar kategori untuk dropdown (sekali muat)
  useEffect(() => {
  ...
  // 7. Profil user (untuk mode admin)
  useEffect(() => {
  ...
  async function confirmSoftDelete() {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        showToast(json.error || "Gagal mengarsipkan burung.", "error");
      }
  ...
  async function confirmHardDelete() {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        showToast(json.error || "Gagal menghapus burung.", "error");
      }
    } catch (err) { showToast(err.message, "error"); } finally { setDeletingId(null); }
  }
    ...
    <div className="bg-gray-50 min-h-screen relative overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <>
                  ...
                  <div key={bird.id} className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden flex flex-col hover:shadow-xl transition-all group">
                    ...
                    <div className="p-4 sm:p-5 flex-grow flex flex-col">
                      <h3 className="text-lg font-bold text-gray-900 mb-0.5 truncate">
                        <Link href={`/bird/${bird.id}`} className="hover:text-blue-600 transition-colors">{bird.name}</Link>
                      </h3>
                      ...
                      {isAdmin ? (
                        <div className="flex gap-2 mt-auto">
                          ...
                          <button onClick={() => {setSelectedBirdId(bird.id); setSelectedBirdName(bird.name); setShowDeleteModal(true);}} aria-label={`Hapus ${bird.name}`} className="flex-1 bg-red-100 text-red-600 py-2.5 rounded-xl flex justify-center items-center hover:bg-red-200 transition active:scale-95">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      ) : (
                        <div className="h-11 mt-auto">
                          {itemInCart ? (
                            <div className="flex items-center justify-between bg-blue-50 border-2 border-blue-600 rounded-xl p-1 h-full">
                              <button onClick={() => updateCartQuantity(bird, -1)} aria-label={`Kurangi ${bird.name}`} className="w-8 h-8 flex items-center justify-center bg-white text-blue-600 rounded-lg shadow-sm hover:bg-red-50 hover:text-red-600 transition"><Minus size={16} /></button>
                              <span className="font-black text-blue-900 text-base">{itemInCart.quantity}</span>
                              <button onClick={() => updateCartQuantity(bird, 1)} aria-label={`Tambah ${bird.name}`} className="w-8 h-8 flex items-center justify-center bg-blue-600 text-white rounded-lg shadow-sm hover:bg-blue-700 transition"><Plus size={16} /></button>
                            </div>
                          ) : (
            ...
            {/* FOOTER PAGINATION */}
            {totalPages > 1 && (
              <div className="mt-10 p-6 bg-white rounded-3xl border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                  Halaman <span className="text-gray-700">{page}</span> dari <span className="text-gray-700">{totalPages}</span>
                </p>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2 rounded-xl bg-gray-50 border border-gray-200 text-gray-400 hover:text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm"
                  >
                    <ChevronLeft size={20} />
                  </button>
      ...
      <div className={`fixed right-0 top-0 h-screen w-[90%] sm:w-[380px] bg-white shadow-2xl z-[70] flex flex-col transition-transform duration-500 ease-in-out ${isCartOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="p-5 sm:p-6 border-b flex items-center justify-between bg-blue-900 text-white">
          ...
          <button onClick={() => setIsCartOpen(false)} aria-label="Tutup keranjang" className="p-2 hover:bg-white/20 rounded-full"><X size={20} /></button>
        </div>
            ...
            <>
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="flex gap-3 sm:gap-4 p-3 bg-gray-50 rounded-2xl border border-gray-100 relative group">
                  <div className="h-16 w-16 sm:h-20 sm:w-20 rounded-xl overflow-hidden bg-gray-100 shrink-0 relative">
                    <BirdImage src={item.image_url} alt={item.name} width={80} height={80} sizes="80px" className="object-cover" />
                  </div>
                  ...
                  <button onClick={() => setCartItems(prev => prev.filter(i => i.id !== item.id))} aria-label={`Hapus ${item.name} dari keranjang`} className="absolute top-2 right-2 p-1 text-gray-300 hover:text-red-500 transition"><Trash2 size={16}/></button>
                </div>
              ))}
            </>
          )}
